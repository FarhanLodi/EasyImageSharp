...
Models are fetched on first use and cached under `%LOCALAPPDATA%/EasyImageSharp/models`
(`~/.local/share` on Linux and macOS).

| Model | Task | Size | Licence | Status |
|---|---|---|---|---|
| `PP-LCNet_x1_0_doc_ori.onnx` | Document orientation | 6.8 MB | Apache-2.0 | Published |
| `UVDoc.onnx` | Page dewarp | 30 MB | MIT | Published |
| `realesrgan_general_x4v3.onnx` | Super-resolution ×4 | 4.9 MB | BSD-3-Clause | Not yet published |
| `dncnn_gray_blind.onnx` | Grayscale denoise | < 1 MB | check upstream | Not yet published |
| `u2netp.onnx` | Saliency / background | 4.7 MB | Apache-2.0 | Not yet published |
| `sauvolanet.onnx` | Learned binarisation | small | check upstream | Not yet published |

Models marked *not yet published* have no pinned checksum in this release. Point the library at your
own export until they ship:
